...
<head>
    ...
    <title>Airbus Cross-Programme Resource Management Dashboard</title>
    <style>
        :root {
...
<body>
    <div class="header">
        <h1>🛫 Airbus Cross-Programme Resource Management Dashboard</h1>
    ...
    <div class="container">
        <div class="tabs">
            ...
            <button class="tab" onclick="switchTab('expertise', event)">👥 Expertise Assessment</button>
            <button class="tab" onclick="switchTab('vendors', event)">🏢 Vendor Matrix</button>
            <button class="tab" onclick="switchTab('allocation', event)">📋 Work Allocation</button>
            <button class="tab" onclick="switchTab('dependency', event)">🔗 Dependency Check</button>
        </div>
    ...
    <script>
        ...
        function calculateExpertise() {
            ...
            if (score >= 8) {
                ...
                recommendation = '⚠️ <strong>Protect this resource:</strong> Limit to max 3 programmes. Consider mentoring junior staff to distribute knowledge. Monitor workload closely.';
            } else if (score >= 5.5) {
                classification = 'Medium Expertise (Capable with Supervision)';
                statusClass = 'status-yellow';
                recommendation = '✓ <strong>Capable resource:</strong> Can handle medium-complexity tasks independently. Suitable for supervised outsourcing review. Can mentor on specific topics.';
            } else {
                classification = 'Low Expertise (Needs Support)';
                statusClass = 'status-green';
                recommendation = '📚 <strong>Development needed:</strong> Assign to routine tasks with senior oversight. Provide training opportunities. Pair with high-expertise mentors.';
            }
        ...
        function recommendAllocation() {
            ...
            const resultHTML = `
                <div class="result-box ${resultClass}">
                    ...
                    <div style="margin-top: 1rem; padding-top: 1rem; border-top: 1px solid rgba(0,0,0,0.1);">
                        <strong>✓ Recommended Resource:</strong> ${resource}<br>
                        <strong>📋 Supervision Level:</strong> ${supervision}<br>
                        <strong>⚠️ Risk Level:</strong> ${risk}
                    </div>
                </div>
            `;
        ...
        function runDependencyCheck() {
            ...
            if (issues.length === 0) {
                ...
                decision = '✅ Task can proceed as planned';
                ...
                decision = '⚠️ Task requires Portfolio PMO review before proceeding';
                actions = '<strong>Detected Issues:</strong><ul style="margin: 0.5rem 0;">' +
                    issues.map(i => `<li>${i}</li>`).join('') +
                    '</ul><strong>Actions:</strong><br>';

                if (q1 === 'yes') actions += '• Consider delaying or reassigning to prevent expert bottleneck<br>';
                if (q2 === 'yes') actions += '• Evaluate alternative vendors or adjust timeline<br>';
                if (q3 === 'yes') actions += '• Convert to supervised internal work with expert review<br>';
                if (q4 === 'yes') actions += '• Coordinate with affected programmes before proceeding<br>';

                resultClass = 'result-review';
            } else {
                status = 'RED - Cannot Proceed';
                statusBadge = 'status-red';
                decision = '🛑 Task BLOCKED - High risk of cross-programme failure';
                actions = '<strong>Critical Issues:</strong><ul style="margin: 0.5rem 0;">' +
                    issues.map(i => `<li>${i}</li>`).join('') +
                    '</ul><strong>Required Actions:</strong><br>' +
                    '• Escalate to Portfolio PMO immediately<br>' +
                    '• Reassess task priority and timeline<br>' +
                    '• Identify alternative resources or approach<br>' +
                    '• Do NOT assign until dependencies resolved';
                resultClass = 'result-stop';
            }
      ...
      vendors.forEach(v => {
        ...
        html += `
          <div style="margin:8px 0; padding:8px; border:1px solid #ddd; border-radius:6px; background:#fff;">
            <strong>${v.name}</strong> — <em>${(v.programmes || []).join(", ")}</em><br>
            Quality: ${v.quality}/10 • Capacity used: ${v.capacity}% • <span class="vendor-tier ${tierClass}">${tier}</span>
          </div>
        `;
      });
